...
* **并行**：922 个探针在默认 dispatcher 上以有限的许可数散开；结果一到达便流入 UI。
...
`#` 开始注释；空行会被忽略。 有些文件每行包含以空格或制表符分隔的几个字段，或可选标记（`props.txt` 中的 `S` 和 `@field`）；请保持文件中已有行的格式。也欢迎代码贡献。提交贡献即表示你同意其以本项目的 AGPL-3.0-or-later 发布。
