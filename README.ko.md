...
`#` 는 주석을 시작하며 빈 줄은 무시됩니다. 일부 파일은 한 줄에 공백 또는 탭으로 구분된 여러 필드나 선택적 표시(`props.txt` 의 `S` 와 `@field`)를 담고 있습니다. 파일에 이미 있는 줄의 형식을 유지하세요. 코드 기여도 환영합니다. 기여하면 귀하의 작업이 이 프로젝트의 AGPL-3.0-or-later 로 배포되는 데 동의하는 것입니다.
